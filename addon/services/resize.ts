...
export default class ResizeService extends Service.extend(Evented, {
  ...
  constructor() {
    ...
    this._onResizeHandler = (evt) => {
      ...
      const scheduledDebounce = debounce(
        this,
        this._fireDebouncedResizeNotification,
        evt,
        <number>this.get('debounceTimeout')
      );
  ...
  public _setDefaults() {
    const defaults = (<any>this).resizeServiceDefaults ?? {};

    Object.keys(defaults).map((key) => {
      const classifiedKey = classify(key);
      const defaultKey = `default${classifiedKey}`;
      return set(this as any, defaultKey, (<any>defaults)[key]);
    });
  }
